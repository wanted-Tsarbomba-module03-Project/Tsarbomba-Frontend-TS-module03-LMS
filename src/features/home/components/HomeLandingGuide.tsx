...
const STEPS = [
  { no: "01", emoji: "🎬", title: "강의 시청", desc: "관심 있는 강의를 골라 학습하세요." },
  { no: "02", emoji: "💻", title: "문제 풀이", desc: "학습한 내용을 문제로 확인하세요." },
  { no: "03", emoji: "🏅", title: "배지 획득", desc: "학습 목표를 달성하고 배지를 모으세요." },
  { no: "04", emoji: "🏆", title: "랭킹 확인", desc: "누적 점수로 순위를 비교하세요." },
];
    ...
    <div className="flex flex-col gap-20 pb-10">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#1a237e] to-[#111751] px-6 py-16 text-center sm:px-12 sm:py-20">
        ...
        <p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-white/70 sm:text-lg">
          codebomba는 강의 · 문제풀이 · 랭킹 · AI 챗봇으로 학습 흐름이 자연스럽게
          이어지는 데이터 분석 학습 플랫폼이에요.
        </p>
      ...
      <section>
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((step) => (
            <div
              key={step.no}
              className="rounded-2xl border border-[#e8e8e8] bg-white p-6 transition-shadow hover:shadow-md"
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-[#1a237e]">{step.no}</span>
          ...
          {FLOW.map((s, i) => (
            ...
            >
              <div className="sm:w-3/5">
                <Shot src={`/assets/img/guide/${s.img}`} alt={`${s.title} 화면`} />
              </div>
              <div className="sm:w-2/5">
                <span className="inline-block rounded-full bg-[#eff6ff] px-3 py-1 text-sm font-bold text-[#1d4ed8]">
                  {s.step}
                </span>
                <h3 className="mt-3 text-xl font-extrabold text-[#1f2937]">{s.title}</h3>
                <p className="mt-3 text-base leading-relaxed text-[#6b7280]">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-3xl bg-[#f3f4f6] px-6 py-14 sm:px-12">
        ...
        <div className="mx-auto mt-10 grid max-w-3xl grid-cols-1 gap-5 sm:grid-cols-2">
          <div className="rounded-2xl border border-[#e8e8e8] bg-white p-6">
            <h3 className="text-lg font-bold text-[#1f2937]">📦 대문제 · 소문제</h3>
            <p className="mt-3 text-sm leading-relaxed text-[#6b7280]">
              문제세트(대문제) 하나는 데이터셋 하나와 여러 개의 소문제로 이뤄져요.
              소문제는 난이도·배점별로 나뉘어 있어 단계적으로 풀어나갈 수 있어요.
            </p>
          </div>
          <div className="rounded-2xl border border-[#e8e8e8] bg-white p-6">
            <h3 className="text-lg font-bold text-[#1f2937]">📊 진짜 데이터로 연습</h3>
            <p className="mt-3 text-sm leading-relaxed text-[#6b7280]">
              교과서 예제가 아니라 공공·의료·금융 등 실제 데이터를 다뤄요.
              결혼이민자 현황·경제지표·흡연단속 실적 같은 데이터로 실무 감각을 키워요.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-3xl bg-[#f3f4f6] px-6 py-14 sm:px-12">
        ...
        <div className="mx-auto mt-10 max-w-2xl">
          <Shot src="/assets/img/guide/ranking-board.png" alt="랭킹 페이지 화면" />
        </div>
      </section>
